...
#   * Keep v1.8.1 improvements; no behavior regressions expected.
...
def _elem_creation_info() -> dict:
    return {
        "type": "CreationInfo",
        "created": _now_iso(),
        "specVersion": "3.0.1",
        "createdBy": [{"type": "Tool", "spdxId": "SPDXRef-TOOL", "name": "hf2spdx-ai-bom", "version": "2.0.0"}]
    }
...
def _get_supplier_org(repo_id: str) -> dict:
    owner = (repo_id.split("/", 1)[0] if "/" in repo_id else repo_id) or "unknown"
    return {"type": "Organization", "spdxId": "SPDXRef-SUPPLIER", "name": owner}
...
def parse_requirements_text(text: str) -> Set[str]:
    """Parse requirements*.txt content. Ignore include/constraint lines and filenames."""
    pkgs: Set[str] = set()
    REQ_MARKERS = {"-r", "--requirement", "-c", "--constraint", "--requirements", "--constraints"}
    FILENAME_EXTS = (".txt", ".in", ".cfg", ".yaml", ".yml")
    for raw in text.splitlines():
        line = raw.strip()
        if not line or line.startswith("#"):
            continue
        # tokens (preserve order to look at markers + the next token)
        toks = re.split(r"\s+", line)
        i = 0
        while i < len(toks):
            t = toks[i].strip().strip(",")
            i += 1
            if not t:
                continue
            tl = t.lower()
            # skip markers like "-r requirements.txt" (also skip the next token = filename)
            if tl in REQ_MARKERS:
                if i < len(toks):  # skip filename after marker
                    i += 1
                continue
            # skip options
            if tl.startswith("-"):
            ...
            if tl.endswith(FILENAME_EXTS):
                continue
            # normalize package token
            tl = re.sub(r"\[.*\]$", "", tl)          # extras
            tl = re.split(r"[<>=!~]", tl)[0]         # version pins
            if not re.match(r"^[a-z0-9][a-z0-9._-]*$", tl):
                continue
            if tl in {"python","pip","setuptools","wheel"}:
                continue
            if tl == "opencv":
                tl = "opencv-python"
            pkgs.add(tl)
            # usually one pkg per line; break to be conservative
            break
    return pkgs



def get_deps_from_repo_python_files(repo_id: str, model_info: Dict[str, Any], timeout: int = 30) -> Set[str]:
    """Scan small .py files in the repo for import lines; add allowlisted libs as dependencies.
    Facts-only: content is fetched from HF raw URLs at the pinned commit. Conservative limits to avoid overfetch.
    """
    commit_sha = model_info.get("sha") or "main"
    allow = set(IMPORT_ALLOWLIST)
    found: Set[str] = set()
    num_scanned = 0
    for f in model_info.get("siblings") or []:
        ...
        if not name.endswith(".py"):
            continue
        size = f.get("size")
        # Only scan reasonably small files (<= 512 KB) or unknown but likely small names.
        if isinstance(size, int) and size > 512 * 1024:
            continue
        # Hard cap to avoid too many requests
        if num_scanned >= 40:
            break
        url = f"{HF_BASE}/{repo_id}/resolve/{commit_sha}/{name}"
        try:
            r = requests.get(url, timeout=timeout)
            ...
            try:
                ...
                if r.status_code == 200 and r.text:
                    if name.lower().startswith("requirements"):
                        ...
                        # very rough: only keep tokens that look like PyPI packages,
                        # and drop conda meta or tool names.
                        deps = re.findall(r"-\s*([A-Za-z0-9_.\-]+)", r.text)
                        for d in deps:
                            low = d.lower()
                            if low in {"python","pip","mamba","conda","setuptools","wheel"}:
                                continue
                            if low.endswith((".txt",".in",".cfg",".yaml",".yml")):
                                continue
                            if low == "opencv":
                                low = "opencv-python"
                            if re.match(r"^[a-z0-9][a-z0-9._-]*$", low):
                                found.add(low)
                    elif name == "pyproject.toml":
                        txt = r.text
                        # project.dependencies = [ "...", "..." ]
                        for m in re.finditer(r"project\\.dependencies\\s*=\\s*\\[(.*?)\\]", txt, flags=re.S):
                            for q in re.findall(r'"([^"]+)"', m.group(1)):
                                pkg = re.split(r"[<>=!~]", q.strip())[0].lower()
                                if pkg and pkg not in {"python"} and re.match(r"^[a-z0-9][a-z0-9._-]*$", pkg):
                                    if pkg == "opencv": pkg = "opencv-python"
                                    found.add(pkg)
                        # [tool.poetry.dependencies] block
                        ...
                        if m:
                            for line in m.group(1).splitlines():
                                ...
                                if key and key not in {"python"} and re.match(r"^[a-z0-9][a-z0-9._-]*$", key):
                                    if key == "opencv": key = "opencv-python"
                                    found.add(key)
...
def build_spdx(repo_id: str,
    ...
    doc = {
        ...
        "creationInfo": {
            ...
            "specVersion": "3.0.1",
            "createdBy": [
                {"type": "Tool", "spdxId": "SPDXRef-TOOL", "name": "hf2spdx-ai-bom", "version": "2.0.0"}
            ],
            "createdUsing": [
                {"type": "Tool", "spdxId": "SPDXRef-TOOL", "name": "hf2spdx-ai-bom", "version": "2.0.0"}
            ]
    ...
    ai_pkg = {
        ...
        "releaseTime": _get_release_time(model_info, doc["creationInfo"]["created"]),
        "suppliedBy": [_get_supplier_org(repo_id)],
        "creationInfo": _elem_creation_info()
    }
    # optional description from pipeline_tag
    if pipeline_tag:
        ai_pkg["description"] = f"{pipeline_tag} model on Hugging Face"
    doc["element"].append(ai_pkg)
    # ensure supplier is materialized as an element
    doc["element"].append(dict(_get_supplier_org(repo_id), **{"creationInfo": _elem_creation_info()}))
    ...
    if deps:
        ...
        def _dep_ok(n: str) -> bool:
            ...
            nl = (n or "").strip().lower()
            if not re.match(r'^[a-z0-9][a-z0-9._-]*$', nl): return False
            if nl.endswith(bad_exts): return False
            if nl in {'requirements','constraints'}: return False
            if nl.startswith('-'): return False
            return True
        deps = { n for n in deps if _dep_ok(n) }
    ...
    if lic_std:
        for sid, rtype in (("SPDXRef-LIC-MODEL-DECLARED","hasDeclaredLicense"),
            ...
            doc["element"].append({"type":"LicenseExpression","spdxId":sid,"licenseExpression":lic_std,"creationInfo":_elem_creation_info()})
            rels.append({"type":"Relationship","spdxId":f"SPDXRef-REL-{sid.split('-')[-1]}",
                         "from":"SPDXRef-MODEL","to":sid,"relationshipType":rtype,"creationInfo":_elem_creation_info()})
    else:
        custom_text = custom_license_text or (license_raw or "Custom license (text unavailable)")
        for sid, rtype in (("SPDXRef-LIC-MODEL-DECLARED","hasDeclaredLicense"),
                           ("SPDXRef-LIC-MODEL-CONCLUDED","hasConcludedLicense")):
            doc["element"].append({"type":"SimpleLicensingText","spdxId":sid,"licenseText":custom_text,"creationInfo":_elem_creation_info()})
            rels.append({"type":"Relationship","spdxId":f"SPDXRef-REL-{sid.split('-')[-1]}",
                         "from":"SPDXRef-MODEL","to":sid,"relationshipType":rtype,"creationInfo":_elem_creation_info()})
    ...
    doc["element"].append({"type":"Relationship","spdxId":"SPDXRef-REL-DESCRIBES","from":"SPDXRef-DOCUMENT","to":"SPDXRef-MODEL","relationshipType":"describes","creationInfo":_elem_creation_info()})
